...
<div class="content-wrapper">
    @include('layouts.content-header', ['name' => 'Trang Chủ', 'key' => 'Cập nhật quyền'])
    <div class="content">
        @if(session('status'))
                        ...
                        <form action="{{route('roles.update',$role->id)}}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class="form-group col-md-12">
                                    ...
                                    <input type="name" placeholder="Tên quyền" name="name_roles" class="form-control" id="name_slider" value="{{$role->name}}">
                                    @error('name_roles')
                                ...
                                <div class="form-group col-md-12">
                                    ...
                                    <textarea name="descript" placeholder="Mô tả quyền" id="" cols="30" rows="10">{{$role->display_name}}</textarea>
                                    @error('descript')
                                    <small style="font-size: 16px; color:red !important;" class="form-text text-danger ">{{$message}}</small>
                                    @enderror
                                </div>
    ...
    border-radius: 5px;">
                                ...
                                <div class="cardss text-dark bg-light mb-3 col-md-12">
                                    ...
                                    <div class="d-flex justify-content-around">
                                        ...
                                        <div class="card-body">
                                            <h5 class="card-title">
                                                <input type="checkbox" class="checkbox_children" name="permission_id[]" {{$permissionChecked->contains('id', $child->id)?'checked':''}} value="{{$child->id}}" id="{{$child->name}}">
                            ...
                            <button class="btn btn-success p-2 text-center d-block">Cập Nhật</button>
                        </form>
                    </div>
                </div>
            </div>
